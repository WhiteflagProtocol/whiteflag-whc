...
function sendMessages(wfMessages, options, callback) {
    ...
    let counter = 0;
    for (let i = 0; i < wfMessages.length; i++) {
        let wfMessageSet = wfMessages[i];

        // Send protective sign first
        counter++;
        if (options.verbose) console.log(`Sending messgae ${counter}/${(wfMessages.length * 2)}: ${messageType(wfMessageSet[0])} for site ${wfMessageSet[0].MetaHeader.whlSiteNumber}`);
        sendMessage(wfMessageSet[0], options, function sendPMessageCb(err, result) {
            if (err) {
                // Error; also skip the related resource message
                console.error(`${err.message}`);
                counter++;
                console.warn(`Skipping message ${(counter)}/${(wfMessages.length * 2)}: ${messageType(wfMessageSet[1])} for site ${wfMessageSet[1].MetaHeader.whlSiteNumber}`);
            } else {
                // Send resource message refering to protective sign
                counter++;
                if (options.verbose) console.log(`Sending message ${counter}/${(wfMessages.length * 2)}: ${messageType(wfMessageSet[1])} for site ${wfMessageSet[1].MetaHeader.whlSiteNumber}`);
