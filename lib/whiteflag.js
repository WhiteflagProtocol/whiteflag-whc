...
function sendMessages(wfMessages, options, callback) {
     ...
     * @private
     */
    function iterateMessages(i = 0) {
        // Return if completed
        if (i >= wfMessages.length) {
            if (skipped.length > 0) {
                return callback(new Error(`Could not sent some or all messages for ${skipped.length} sites: ${JSON.stringify(skipped)}`));
            }
            return callback(null);
        }
        // Count messages and get next one to send
        let j = (i * 2) + 1;
        let wfMessageSet = wfMessages[i];

        // Send protective sign first
        if (options.verbose) console.log(`Sending message ${j}/${(wfMessages.length * 2)}: ${messageType(wfMessageSet[0])} for site ${wfMessageSet[0].MetaHeader.whlSiteNumber}`);
        sendMessage(wfMessageSet[0], options, function sendPMessageCb(err, result) {
            if (err) {
                // Error; skip the related resource message
                console.error(`${err.message}`);
                console.warn(`Skipping message ${(j + 1)}/${(wfMessages.length * 2)}: ${messageType(wfMessageSet[1])} for site ${wfMessageSet[1].MetaHeader.whlSiteNumber}`);
                skipped.push(wfMessageSet[0].MetaHeader.whlSiteNumber);
                return iterateMessages(++i, skipped, callback);
            } else {
                ...
                if (options.verbose) console.log(`Sending message ${(j + 1)}/${(wfMessages.length * 2)}: ${messageType(wfMessageSet[1])} for site ${wfMessageSet[1].MetaHeader.whlSiteNumber}`);
                sendMessage(wfMessageSet[1], options, function sendRMessageCb(err) {
                    if (err) {
                        console.error(err.message);
